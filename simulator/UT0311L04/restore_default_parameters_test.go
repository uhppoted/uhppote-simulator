...
func TestRestoreDefaultParameters(t *testing.T) {
	...
	s.Doors.SetMode(1, types.ModeNormallyOpen)
	s.Doors.SetMode(2, types.ModeNormallyClosed)
	s.Doors.SetMode(3, types.ModeNormallyOpen)
	s.Doors.SetMode(4, types.ModeNormallyClosed)
	...
	if response, err := s.restoreDefaultParameters(&request); err != nil {
		...
		for _, door := range []uint8{1, 2, 3, 4} {
			if s.Doors.Mode(door) != types.ModeControlled {
				t.Errorf("'restore-default-parameters' failed to reset door %v mode\n   expected: %+v\n   got:      %+v\n", door, types.ModeControlled, s.Doors.Mode(door))
			}
